...
module.exports = {
  ...
  execute: async (member) => {
    ...
    try {
      ...
      console.error('Error occurred while fetching data from the API : L\'utilisateur n\'a pas d\'identifiants discord associé ou ', error);
      member.user.send("\:x: Je ne parviens pas à vous assigner le nom et prénom RP sur le serveur.\nJe vous ai donc ouvert un ticket pour régler cela avec l'administration.")
    ...
    if(welcomeChannelID !== "") {
      ...
      await channel.send(`\:wave: Bienvenue **<@${member.user.id}>** dans la **Bianchi**`);
    }
  }
};
